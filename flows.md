...
flowchart TD
  ...
  %% ── ДОДАТИ результат (створює Вимірювання; новий Показник — через довідник) ──
  list --> add["Додати результат аналізу — ➕"]
  ...
  point --> done
  one --> addmore["CTA «додати ще результат»"]
  addmore --> add
...
(без норм/зон)` · `Додати результат аналізу`. Не окремий екран: **довідник показників** —
службовий reference (автодоповнення) всередині «Додати результат».
...
≥1 вимірювання** (створюється разом із першим результатом) → екран крива «n=0» не
